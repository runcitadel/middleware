...
export function toHexString(byteObject) {
...
export async function addInvoice(amt, memo) {
...
export async function closeChannel(txHash, index, force) {
...
export async function estimateChannelOpenFee(amt, confTarget, sweep) {
    ...
    if (confTarget === 0) {
        ...
        for (const key of keys) {
            // @ts-expect-error
            let baseFeeEstimate_key = baseFeeEstimate[key];
            if (baseFeeEstimate_key.feeSat) {
                baseFeeEstimate_key.feeSat = String(parseInt(baseFeeEstimate_key.feeSat, 10) +
                    OPEN_CHANNEL_EXTRA_WEIGHT * baseFeeEstimate_key.feerateSatPerByte);
            }
        }
    }
    else if (baseFeeEstimate.feeSat) {
        baseFeeEstimate.feeSat = String(parseInt(baseFeeEstimate.feeSat, 10) +
...
export async function estimateFee(address, amt, confTarget, sweep) {
...
export async function estimateFeeSweep(address, fullAmtToEstimate, mempoolMinFee, confTarget, l, r) {
    const amtToEstimate = parseInt(l) +
        Math.floor((parseInt(r) - parseInt(l)) / 2); // eslint-disable-line no-magic-numbers
    try {
        ...
        if (l === amtToEstimate) {
            ...
            const estimatedFeeSatPerKiloByte = parseInt(successfulEstimate.feerateSatPerByte) * 1000;
            if (estimatedFeeSatPerKiloByte <
...
export async function estimateFeeGroupSweep(address, amt, mempoolMinFee) {
    const calls = [
        estimateFeeSweep(address, amt, mempoolMinFee, FAST_BLOCK_CONF_TARGET, 0, amt),
    ...
    const [fast, normal, slow, cheapest] = await Promise.all(calls.map((p) => p.catch((error) => handleEstimateFeeError(error))));
    return {
        fast: fast,
        normal: normal,
        slow: slow,
...
export async function estimateFeeWrapper(address, amt, mempoolMinFee, confTarget) {
    const estimate = await lndService.estimateFee(address, amt, confTarget);
    const estimatedFeeSatPerKiloByte = parseInt(estimate.feerateSatPerByte) * 1000;
    if (estimatedFeeSatPerKiloByte < convert(mempoolMinFee, "btc", "sat", "Number")) {
        throw new NodeError("FEE_RATE_TOO_LOW");
    }
    return estimate;
}
export async function estimateFeeGroup(address, amt, mempoolMinFee) {
    const calls = [
...
export function handleEstimateFeeError(error) {
...
export function getForwardingEvents(startTime, endTime, indexOffset) {
...
export function getTxnHashFromChannelPoint(channelPoint) {
...
export async function getChannels() {
    ...
    for (const channel of pendingChannels.pendingOpenChannels) {
        ...
        if (channel.initiator) {
            channel.channel.localBalance = String(parseInt(channel.channel.localBalance, 10) +
                parseInt(channel.commitFee, 10));
        }
        else {
            channel.channel.remoteBalance = String(parseInt(channel.channel.remoteBalance, 10) +
                parseInt(channel.commitFee, 10));
        }
        allChannels.push(channel);
    }
    ...
    for (const channel of openChannels) {
        ...
        // Include commitFee in balance. This helps us avoid the leaky sats issue by making balances more consistent.
        if (channel.initiator) {
            channel.localBalance = String(parseInt(channel.localBalance, 10) + parseInt(channel.commitFee, 10));
        }
        else {
            channel.remoteBalance = String(parseInt(channel.remoteBalance, 10) + parseInt(channel.commitFee, 10));
        }
        allChannels.push(channel);
    }
...
export async function getPendingChannelDetails(channelType, pubKey) {
...
export async function getSyncStatus() {
    ...
    if (!info.syncedToChain) {
        ...
        percentSynced = ((info.bestHeaderTimestamp - genesisTimestamp) /
            (currentTime - genesisTimestamp)).toFixed(4); // eslint-disable-line no-magic-numbers
...
export async function initializeWallet(seed) {
    const lndStatus = await getStatus();
    if (lndStatus.operational) {
...
export async function openChannel(pubKey, ip, port, amt, satPerByte, _name, _purpose) {
...
export async function payInvoice(paymentRequest, amt) {
...
export function sendCoins(addr, amt, satPerByte, sendAll) {
...
export async function getNodeAlias(pubkey) {
