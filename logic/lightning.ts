...
import type { Channel, ChannelBalanceResponse, ChannelFeeReport, EstimateFeeResponse, ForwardingHistoryResponse, GetInfoResponse, Invoice, NewAddressResponse, Payment, PendingChannelsResponse, PendingChannelsResponse_ForceClosedChannel, PendingChannelsResponse_PendingChannel, PendingChannelsResponse_PendingOpenChannel, PendingChannelsResponse_WaitingCloseChannel, SendCoinsResponse, SendResponse, Transaction, WalletBalanceResponse } from "../services/lightning/autogenerated-types";
...
export async function decodePaymentRequest(paymentRequest: string): Promise<extendedPaymentRequest> {
...
export async function estimateChannelOpenFee(
  ...
  if (confTarget === 0) {
    const baseFeeEstimateTyped = <Record<string, EstimateFeeResponseExtended>>baseFeeEstimate;
    ...
    for (const key of keys) {
      ...
      if (baseFeeEstimate_key.feeSat) {
        baseFeeEstimate_key.feeSat = String(
          parseInt(<string>baseFeeEstimate_key.feeSat, 10) +
            OPEN_CHANNEL_EXTRA_WEIGHT * (<EstimateFeeResponse>baseFeeEstimate_key).satPerVbyte
        );
      }
    }
  }
...
export async function getForwardingEvents(startTime: number, endTime: number, indexOffset: number): Promise<ForwardingHistoryResponse> {
...
export async function getChannels(): Promise<Channel_extended[]> {
  ...
  for (const channel of pendingChannels.pendingOpenChannels) {
    ...
    if ((<PendingOpenChannel_extended>channel).initiator) {
      ...
      // @ts-expect-error The property exists!
      channel.channel.remoteBalance = (channel.channel?.remoteBalance || 0) + channel.commitFee
    }
  ...
  for (const channel of allChannels as any[]) {
    ...
    if (channel.channel) {
      ...
      const knownChannel =
        chainTxns ? chainTxns[getTxnHashFromChannelPoint(channel.channelPoint)] : null;
      if (!knownChannel) {
...
export async function getSyncStatus(): Promise<{
  ...
  if (!info.syncedToChain) {
    const genesisTimestamp = info.chains[0].network == "testnet"
...
export async function updateChannelPolicy(global: boolean, fundingTxid: string, outputIndex: number, baseFeeMsat: number, feeRate: number, timeLockDelta: number): Promise<void> {
  await lndService.updateChannelPolicy(global, fundingTxid, outputIndex, baseFeeMsat, feeRate, timeLockDelta);
}
