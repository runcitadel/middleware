...
import {encode as encodeLndConnect} from 'simple-lndconnect';
import getBitcoin from '../services/bitcoin.js';
...
export async function estimateFee(
  ...
  EstimateFeeResponseExtended | Record<string, EstimateFeeResponseExtended>
> {
  const mempoolInfo = await bitcoind.getMempoolInfo();
...
export async function getLndConnectUrls(): Promise<{
  restTor: string;
  restLocal: string;
  grpcTor: string;
  grpcLocal: string;
  ...
  const restTor = encodeLndConnect({
    host: restTorHost,
    cert,
    macaroon,
  });
  ...
  const grpcTor = encodeLndConnect({
    host: grpcTorHost,
    cert,
    macaroon,
  });
  ...
  const restLocal = encodeLndConnect({
    host: restLocalHost,
    cert,
    macaroon,
  });
  ...
  const grpcLocal = encodeLndConnect({
    host: grpcLocalHost,
    cert,
    macaroon,
  });
