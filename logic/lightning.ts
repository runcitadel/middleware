...
export const decodePaymentRequest = lndService.decodePaymentRequest;
...
export async function estimateChannelOpenFee(
  ...
  if (confTarget === 0) {
    ...
    for (const key of keys) {
      // @ts-expect-error Object indexing in TS is annoying
      const baseFeeEstimate_key = baseFeeEstimate[key];
...
export const generateAddress = lndService.generateAddress;
...
export async function getChannels(): Promise<Channel_extended[]> {
  ...
  if (chainTxnCall !== null) {
    ...
    for (const txn of chainTxnList) {
      // @ts-expect-error Indexing objects...
...
export const getPendingChannels = lndService.getPendingChannels;
...
export const getGeneralInfo = lndService.getInfo;
...
export const getWalletBalance = lndService.getWalletBalance;
