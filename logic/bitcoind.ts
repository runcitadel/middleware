...
export async function getTransaction(txid: string) {
  ...
  if(typeof transactionObj === 'string') throw new BitcoindError("Received unexpected data from Bitcoin Core");
  // @ts-expect-error
  let vintxid: string = Array.isArray(transactionObj.vin) ? transactionObj.vin[0].txid : transactionObj.vin.txid;
  let vouttx: unknown = Array.isArray(transactionObj.vout) ? transactionObj.vout[0] : transactionObj.vout;
  return {
    txid: txid,
...
export async function getBlock(hash: string) {
  ...
  if(typeof blockObj === 'string') throw new BitcoindError("Received unexpected data from Bitcoin Core");
  return {
    block: hash,
...
export async function getBlocks(fromHeight: number, toHeight: number) {
  ...
  for (let currentHeight = toHeight; currentHeight >= fromHeight; currentHeight--) {
    ...
    if(typeof block === 'string') throw new BitcoindError("Received unexpected data from Bitcoin Core");
