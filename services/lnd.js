...
const lnrpcProto = grpc.loadPackageDefinition(lndPackageDefinition);
...
export async function promiseify(rpcObj, rpcFn, payload, description) {
...
export async function initializeRPCClient() {
    ...
    const walletState = await promiseify(stateService, stateService.getState, {}, "get wallet state");
...
export async function expectWalletToExist() {
    const client = await initializeRPCClient();
    if (!client.Lightning)
        throw new LndError("Error: Wallet not ready");
    return client;
}
// an amount, an options memo, and can only be paid to node that created it.
export async function addInvoice(amount, memo) {
    const rpcPayload = {
        value: amount,
        memo,
        expiry: 3600
...
export async function closeChannel(fundingTxId, index, force) {
    const rpcPayload = {
        channelPoint: {
...
export async function connectToPeer(pubKey, ip, port) {
    const rpcPayload = {
        addr: {
            pubkey: pubKey,
            host: `${ip}:${port}`,
...
export async function decodePaymentRequest(paymentRequest) {
    const rpcPayload = {
...
export async function estimateFee(address, amt, confTarget) {
    const addrToAmount = {};
...
export async function getForwardingEvents(startTime, endTime, indexOffset) {
    const rpcPayload = {
        startTime,
        endTime,
        indexOffset,
...
export async function getNodeInfo(pubkey, includeChannels) {
    const rpcPayload = {
        pubKey: pubkey,
        includeChannels
    };
...
export async function openChannel(pubKey, amt, satPerByte) {
    const rpcPayload = {
...
export async function sendCoins(addr, amt, satPerByte, sendAll) {
    const rpcPayload = {
        addr,
        amount: amt,
        sendAll,
...
export async function sendPaymentSync(paymentRequest, amt) {
    const rpcPayload = {
        paymentRequest,
        amt,
    };
...
export async function unlockWallet(password) {
    ...
    const { WalletUnlocker, state } = await initializeRPCClient();
    if (state == WalletState.UNLOCKED || state == WalletState.RPC_ACTIVE)
        return;
    return await promiseify(WalletUnlocker, WalletUnlocker.UnlockWallet, rpcPayload, 'unlock wallet');
}
export async function updateChannelPolicy(global, fundingTxid, outputIndex, baseFeeMsat, feeRate, timeLockDelta) {
    const rpcPayload = {
        baseFeeMsat,
        feeRate,
        timeLockDelta,
    ...
    const { Lightning } = await expectWalletToExist();
    return await promiseify(Lightning, Lightning.UpdateChannelPolicy, rpcPayload, 'update channel policy coins');
}
