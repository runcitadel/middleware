...
router.get('/', auth.jwt, safeHandler((req, res) => lightningLogic.getChannels()
...
router.get('/pending', auth.jwt, safeHandler((req, res) => lightningLogic.getPendingChannels()
    .then(channels => res.json(channels))));
router.get('/policy', auth.jwt, safeHandler((req, res) => lightningLogic.getChannelPolicy()
...
router.put('/policy', auth.jwt, safeHandler((req, res, next) => {
    ...
    return lightningLogic.updateChannelPolicy(global, fundingTxid, parseInt(outputIndex, 10), baseFeeMsat, feeRate, timeLockDelta)
        // @ts-expect-error
        .then(res.json());
}));
...
router.get('/count', auth.jwt, safeHandler((req, res) => lightningLogic.getChannelCount()
